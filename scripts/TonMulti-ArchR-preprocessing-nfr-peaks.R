...
n_threads <- 15
genome <- "hg38"
data_dir <- "/lustre/scratch/kiviaho/spatac/data/tonsilatlas/multiome/"  # Remember to create an ArchR folder inside
inputFiles <- c("/lustre/scratch/kiviaho/spatac/data/tonsilatlas/multiome/BCLLATLAS_42_43/ulx1v6sz_8a2nvf1c/atac_fragments.tsv.gz")
names(inputFiles) <- c("tonsil_mutiome")
proj_name <- "tonsil_mutiome"
...
# Input files
setwd(sprintf("%s/ArchR", data_dir)) # Remember to create an ArchR folder inside
